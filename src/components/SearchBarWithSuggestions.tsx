...
// ── Where a country/division match should navigate. Goes to the
// ExploreCountries cards page (pre-selecting that country) rather than the
// homepage, so picking a result always lands on an actual country/division
// card view instead of the generic home feed.
//
// ASSUMPTION — adjust EXPLORE_COUNTRIES_PATH if ExploreCountries.tsx is
// mounted at a different route in your router. ──
const EXPLORE_COUNTRIES_PATH = "/explore-countries";
const regionCountryPath = (countryName: string) => `${EXPLORE_COUNTRIES_PATH}?country=${encodeURIComponent(countryName)}`;
const regionDivisionPath = (countryName: string, divisionId: string) =>
  `${EXPLORE_COUNTRIES_PATH}?country=${encodeURIComponent(countryName)}&division=${divisionId}`;
