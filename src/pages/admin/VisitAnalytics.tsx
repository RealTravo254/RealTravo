...
// The generated Supabase types don't know about our custom functions,
...
export default function VisitAnalytics() {
  ...
  // 1. Admin check. The server decides (user_roles table); the page just asks.
  useEffect(() => {
    if (!user) {
      navigate("/auth");
      return;
    ...
    let cancelled = false;
    (async () => {
      const { data, error: err } = await db.rpc("can_view_analytics");
      if (cancelled) return;
      if (err || data !== true) {
        setAllowed(false);
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 px-4 pt-4 pb-12 max-w-4xl mx-auto w-full space-y-5">
        {/* Title bar */}
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            <button
              onClick={() => navigate("/account")}
              className="h-8 w-8 rounded-lg border border-border bg-card flex items-center justify-center hover:bg-muted shrink-0"
              aria-label="Back"
            >
              <ArrowLeft className="h-4 w-4" />
            </button>
            <div className="min-w-0">
              <h1 className="text-base font-bold text-foreground truncate">Visitor Analytics</h1>
              <p className="text-[10px] text-muted-foreground">Web and app visits · Admin only</p>
            </div>
          </div>
          <button
            onClick={load}
                  ...
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-muted text-muted-foreground">
                      <tr className="text-left">
                        <th className="px-3 py-2 font-bold">{view.group === "day" ? "Date" : view.group === "month" ? "Month" : "Year"}</th>
