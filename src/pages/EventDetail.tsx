...
    if (navigator.share) {
      try {
        await navigator.share({ title: event?.name, text: `Check out this event: ${event?.name}`, url: refLink });
      } catch (error) {
...
        <Header />
        {/* Loading state container now uses full width padding */}
...
                
                {/* START: BACK BUTTON - Positioned absolutely over the carousel */}
                <Button 
                    variant="ghost" 
                    onClick={() => navigate(-1)} 
...
                
                {event.description && (
                    <div className="absolute bottom-0 left-0 right-0 bg-black/70 backdrop-blur-sm text-white p-4 z-10 sm:p-2">
...
            {/* --- Activities Section (Full width of the inner max-w-7xl container) --- */}
