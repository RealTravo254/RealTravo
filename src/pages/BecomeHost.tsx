    ...
    <div className="bg-white rounded-[28px] overflow-hidden shadow-xl border border-slate-100">
      ...
      <div className="p-5 space-y-3">
        <div className="flex items-start gap-2.5 p-3.5 rounded-2xl border bg-emerald-50 border-emerald-200">
          ...
          <div>
            <p className="text-[11px] font-black text-emerald-700 uppercase tracking-wide">Live and accepting bookings</p>
            <p className="text-[11px] text-emerald-600 font-medium mt-0.5 leading-snug">
              Your adventure place is visible to explorers. Guests can now discover and book your experience.
            </p>
          </div>
        </div>
  ...
  return (
    <div className="bg-white rounded-[28px] overflow-hidden shadow-xl border border-slate-100">
      ...
      <div className="p-5">
        ...
        {/* Info note — no other action while pending */}
        <div className="mt-4 flex items-start gap-2 px-1">
          <Info className="h-3.5 w-3.5 text-slate-400 mt-0.5 shrink-0" />
          <p className="text-[10px] text-slate-400 leading-relaxed">
            You cannot submit another adventure place or switch hosting type while your submission is under review.
          </p>
        </div>
      </div>
    </div>
...
const BecomeHost = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (profileData?.is_banned) { setIsBanned(true); setLoading(false); return; }
        if (profileData && !profileData.profile_completed) { navigate("/complete-profile"); return; }

        // ── 2. Verification & company status ─────────────────────────────
        ...
        const currentVerStatus = verification?.status || null;

        setVerificationStatus(currentVerStatus);
        ...
        if (currentCategory === "adventure") {
          ...
          if (advPlaces && advPlaces.length > 0) {
            ...
            const { data: hotelPlaces } = await supabase
              ...
              .limit(1);
            if (hotelPlaces && hotelPlaces.length > 0) place = hotelPlaces[0];
          }
        ...
        // ── 4. Non-adventure path ─────────────────────────────────────────
        const isApprovedGuide   = currentVerStatus === "approved";
        const isApprovedCompany = company?.verification_status === "approved";

        if (!verification && !company) { setShowTypeSelection(true); setLoading(false); return; }
        if (!isApprovedGuide && !isApprovedCompany) { setShowTypeSelection(true); setLoading(false); return; }
  ...
  const handleHostTypeSelect = (type: HostType) => {
    if (type === "adventure") navigate("/create-adventure");
    else navigate(`/host-verification?category=${type}`);
  };
  ...
  if (hostingCategory === "adventure") {
    ...
    // ── No place submitted yet (category=adventure but no place found) ─────
    return (
      <div className="min-h-screen bg-[#F8F9FA] flex flex-col">
        <Header />
        <main className="flex-1 container px-4 py-12 mx-auto mb-24 max-w-2xl">
