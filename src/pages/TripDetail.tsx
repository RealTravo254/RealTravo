            ...
            {images.map((img, idx) => (
              <button
                ...
                style={{
                  width: 56,
                  height: 42,
                  outline: idx === current ? `2px solid ${CORAL}` : "2px solid transparent",
                  outlineOffset: 1,
  ...
  return (
    <>
      ...
      <div className="hidden md:block max-w-6xl mx-auto px-4 pt-4">
        ...
        >
          <div
            style={{ gridRow: "1 / 3", overflow: "hidden", borderRadius: 0, cursor: "pointer" }}
            onClick={() => open(0)}
          >
            {images[0] && (
              <img
          ...
          <div
            style={{ overflow: "hidden", borderRadius: 0, position: "relative", cursor: "pointer" }}
            onClick={() => open(2)}
          >
            {images[2] && (
              <img
    ...
    <section className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-black uppercase tracking-tight" style={{ color: TEAL }}>
              Location
            </h2>
...
const TripDetail = () => {
  ...
  const handleCopyLink = async () => {
    ...
    await navigator.clipboard.writeText(
      getShareLink(event.id, "trip", event.name, event.location)
    );
  ...
  const handleBookingSubmit = async (data: BookingFormData) => {
    ...
    try {
      const totalAmount =
        data.num_adults * event.price + data.num_children * (event.price_child || 0);
      await submitBooking({
        itemId: event.id,
        itemName: event.name,
        bookingType: "trip",
      ...
      <main className="container px-4 max-w-6xl mx-auto mt-5 relative z-10">
        <div className="grid lg:grid-cols-[1.7fr,1fr] gap-6">
          ...
          <div className="space-y-5">
            ...
            {/* ── Map Section — always open with Google Maps link ── */}
            ...
            <div className="bg-white rounded-[28px] p-5 shadow-2xl border border-slate-100 lg:sticky lg:top-24">
                ...
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <span
                    className={`text-[10px] font-black uppercase ${
                      remainingSlots < 5 ? "text-red-500" : "text-emerald-600"
                    }`}
                  >
                ...
                <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                  <div
                    className={`h-full transition-all duration-500 ${
                      remainingSlots < 5 ? "bg-red-500" : "bg-emerald-500"
                    }`}
                    style={{
                      width: `${Math.min(
                        (remainingSlots / (event.available_tickets || 50)) * 100,
                        100
                      )}%`,
                    }}
                  />
              ...
              <div className="space-y-2 mb-4">
                ...
                <div className="flex justify-between text-xs font-bold uppercase tracking-tight">
                  ...
                  <span
                    className={
                      event.allow_children === false ? "text-red-500" : "text-emerald-600"
                    }
                  >
                ...
                {event.ticket_types?.length > 0 && (
                  <div className="pt-2 border-t border-slate-100">
                    ...
                    {event.ticket_types.map((ticket: any, i: number) => (
                      <div
                        key={i}
                        className="flex justify-between text-xs font-bold uppercase tracking-tight py-0.5"
                      >
              ...
              <Button
                ...
                style={{
                  background: !canBook
                    ? "#cbd5e1"
                    : `linear-gradient(135deg, ${CORAL_LIGHT} 0%, ${CORAL} 100%)`,
                }}
              >
