...
// Paystack injects iframes at the CSS max z-index so nothing in the normal
// stacking context can sit above them. We fight back by appending a div directly
// to <body> via a React portal and giving it the same z-index + pointer-events.
  ...
  return createPortal(
    <div
      style={{
        ...
        left: 0,
        right: 0,
        zIndex: 2147483647,          // same level as Paystack iframes
        pointerEvents: "auto",
...
const BookingPage = () => {
  ...
  // When Paystack's full-screen iframe is active, push the page content down
  // so the portal header doesn't overlap the payment form itself.
  useEffect(() => {
    if (showPaystackContainer && !isCompleted && !isVerifying) {
      // Give the body a top-padding equal to ~64px so Paystack's own content
      // starts below our floating header
  ...
  // Whether the Paystack full-screen overlay is active
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      {/* ── Portal header: floats ABOVE Paystack's z-index:2147483647 iframes ──
          Only shown when Paystack's full-screen overlay is covering the page.   */}
      {paystackIsActive && item && (
        <PaystackFloatingHeader
          itemName={item.name}
          onBack={handlePaystackBack}
        />
      )}
      ...
      {/* Paystack Inline Checkout Container — Paystack mounts its iframe here,
          then immediately expands it to cover the full viewport. The portal
          header above stays on top of that expansion.                          */}
      {showPaystackContainer && !isCompleted && !isVerifying && (
