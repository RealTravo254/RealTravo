...
const ImageGalleryModal = ({
  ...
  const modal = (
    <div style={{ position: "fixed", inset: 0, background: "#000000", display: "flex", flexDirection: "column", zIndex: 2147483647, paddingTop: "env(safe-area-inset-top, 0px)", paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
      ...
      <div style={{ flex: 1, position: "relative", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", padding: "0 48px" }}>
        <img key={current} src={images[current]} alt={`${name} ${current + 1}`} style={{ maxHeight: "100%", maxWidth: "100%", objectFit: "contain", borderRadius: 0, userSelect: "none", display: "block" }} />
        {images.length > 1 && (
          <>
            <button onClick={() => setCurrent((p) => (p - 1 + images.length) % images.length)}
              style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.10)", backdropFilter: "blur(4px)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
              onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(255,255,255,0.20)")}
              onMouseLeave={(e) => (e.currentTarget.style.background = "rgba(255,255,255,0.10)")}>
              <ChevronLeft style={{ width: 20, height: 20, color: "#fff" }} />
            </button>
            <button onClick={() => setCurrent((p) => (p + 1) % images.length)}
              style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.10)", backdropFilter: "blur(4px)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
              onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(255,255,255,0.20)")}
              onMouseLeave={(e) => (e.currentTarget.style.background = "rgba(255,255,255,0.10)")}>
          ...
          <div style={{ display: "flex", gap: 6, width: "max-content", margin: "0 auto" }}>
            {images.map((img, idx) => (
              <button key={idx} onClick={() => setCurrent(idx)}
                style={{ flexShrink: 0, width: 56, height: 42, padding: 0, border: idx === current ? `2px solid ${CORAL}` : "2px solid rgba(255,255,255,0.22)", borderRadius: 0, outline: "none", opacity: idx === current ? 1 : 0.5, cursor: "pointer", overflow: "hidden", boxSizing: "border-box" }}>
...
const MobileCarousel = ({ images, name }: { images: string[]; name: string }) => {
  ...
  if (!images.length) return (
    <div className="md:hidden w-full bg-slate-200 flex items-center justify-center text-slate-400 font-black uppercase text-xs" style={{ height: "45vh", minHeight: "200px", maxHeight: "360px" }}>No Image</div>
  );
  ...
  return (
    <>
      ...
      <div className="md:hidden relative overflow-hidden bg-slate-900" style={{ height: "45vh", minHeight: "200px", maxHeight: "360px" }}>
        ...
        {images.length > 1 && (
          <>
            <button onClick={() => go(active - 1)} className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center"><ChevronLeft className="h-4 w-4 text-white" /></button>
            <button onClick={() => go(active + 1)} className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center"><ChevronRight className="h-4 w-4 text-white" /></button>
          </>
        )}
        {images.length > 1 && (
...
const InlineFacilitiesGrid = ({ facilities }: { facilities: any[] }) => {
  ...
  const allFacilityImages: string[] = facilities.flatMap((f: any) => Array.isArray(f.images) ? f.images.filter(Boolean) : []);
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            {allFacilityImages.length > 0 && (
              <button onClick={openSectionGallery} className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border transition-all" style={{ color: TEAL, borderColor: `${TEAL}40`, background: `${TEAL}0D` }}>
                <Grid2X2 className="h-3 w-3" /> See All Photos
              </button>
            )}
            {facilities.length > 6 && (
              <button onClick={() => setShowAll((v) => !v)} className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border transition-all" style={{ color: TEAL, borderColor: `${TEAL}40`, background: `${TEAL}0D` }}>
            ...
            return (
              <div key={i} className="bg-white overflow-hidden shadow-sm border border-slate-100" style={{ borderRadius: 0 }}>
                {imgs.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center bg-slate-100" style={{ height: CARD_IMG_HEIGHT }}><MapPin className="h-5 w-5 text-slate-300" /></div>
                )}
                <div className="p-2">
  ...
  return (
    <div className="bg-white overflow-hidden shadow-sm border border-slate-100" style={{ borderRadius: 0 }}>
      <div className="relative overflow-hidden" style={{ height: CARD_IMG_HEIGHT, borderRadius: 0, cursor: imgs.length > 0 ? "pointer" : "default" }} onClick={imgs.length > 0 ? onImageClick : undefined}>
        {imgs.length > 0
          ? imgs.map((img, idx) => <img key={idx} src={img} alt={act.name} className="absolute inset-0 w-full h-full object-cover transition-opacity duration-500" style={{ opacity: active === idx ? 1 : 0, borderRadius: 0 }} />)
        ...
        {imgs.length > 1 && (
          <>
            ...
            <div className="absolute bottom-1 right-1.5 flex gap-0.5 z-20 pointer-events-none">
              {imgs.map((_, idx) => <span key={idx} className="transition-all duration-300 block" style={{ width: active === idx ? "10px" : "3px", height: "3px", borderRadius: "2px", background: active === idx ? "white" : "rgba(255,255,255,0.4)" }} />)}
            </div>
          </>
        )}
      </div>
...
const InlineActivitiesGrid = ({ activities }: { activities: any[] }) => {
  ...
  const allActivityImages: string[] = activities.flatMap((a: any) => Array.isArray(a.images) ? a.images.filter(Boolean) : []);
  const openCardGallery = (imgs: string[], name: string, startIdx = 0) => { setModalImages(imgs); setModalName(name); setModalStart(startIdx); };
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            {allActivityImages.length > 0 && (
              <button onClick={openSectionGallery} className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border transition-all" style={{ color: CORAL, borderColor: `${CORAL}40`, background: `${CORAL}0D` }}>
                <Grid2X2 className="h-3 w-3" /> See All Photos
              </button>
            )}
            {activities.length > 6 && (
              <button onClick={() => setShowAll((v) => !v)} className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border transition-all" style={{ color: CORAL, borderColor: `${CORAL}40`, background: `${CORAL}0D` }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {visibleActivities.map((act: any, i: number) => {
            ...
            return <ActivityCard key={i} act={act} imgs={imgs} onImageClick={imgs.length > 0 ? () => openCardGallery(imgs, act.name, 0) : undefined} />;
          })}
        </div>
      </section>
    </>
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: `${CORAL}18` }}><Zap className="h-3.5 w-3.5" style={{ color: CORAL }} /></div>
      ...
      <div className="flex flex-wrap gap-2">
        {activities.map((act: any, i: number) => {
          ...
          return (
            <div key={i} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl border transition-all hover:scale-[1.03] hover:shadow-sm" style={{ background: p.bg, borderColor: p.border }}>
...
// ── Map Section ─────────────────────────────────────────────────────────────
const MapSection = ({ name, latitude, longitude, location, country, mapLink }: {
  ...
  const googleMapsUrl = hasCoords ? `https://www.google.com/maps?q=${latitude},${longitude}` : mapLink || `https://www.google.com/maps/search/?api=1&query=${searchQuery}`;
  const embedUrl = hasCoords
    ? `https://maps.google.com/maps?q=${latitude},${longitude}&z=15&output=embed`
    : coordMatch ? `https://maps.google.com/maps?q=${coordMatch[1]},${coordMatch[2]}&z=15&output=embed`
...
interface AdminCardProps {
  item: any; creator: any; isBanned: boolean; type: string;
  onOpenMaps: () => void; onApprove: () => void; onReject: () => void; onToggleBan: () => void;
}
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-200 space-y-4 lg:sticky lg:top-24">
      ...
      <div>
        ...
        {price > 0
          ? <div className="flex items-baseline gap-1"><span className="text-3xl font-black text-slate-900">KSh {Number(price).toLocaleString()}</span><span className="text-sm text-slate-500">/ {isAdventure ? "person" : "adult"}</span></div>
          : <span className="text-xl font-bold text-emerald-600">Free Entry</span>}
        {childPrice != null && childPrice > 0 && <p className="text-sm text-slate-600 mt-0.5">Child: KSh {Number(childPrice).toLocaleString()}</p>}
      </div>
      <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
        {(item.opening_hours || item.closing_hours) && (
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-xs font-black text-slate-700">{item.opening_hours === "00:00" && item.closing_hours === "23:59" ? "Open 24 Hours" : `${item.opening_hours || "08:00"} – ${item.closing_hours || "18:00"}`}</span>
          </div>
        )}
        ...
        {item.date && (
          <div className="flex justify-between items-center mt-2 pt-2 border-t border-slate-200">
            ...
            <span className="text-xs font-black text-slate-700">{item.is_custom_date ? <span className="text-emerald-600">Flexible</span> : new Date(item.date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</span>
          </div>
        )}
      ...
      <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-2">
        ...
        {isBanned && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-100 text-red-600 text-[9px] font-black uppercase"><Ban className="h-3 w-3" /> Banned</span>}
        <div className="flex items-center gap-2 text-xs text-slate-500"><Mail className="h-3 w-3 text-teal-600" /><span>{item.email || creator?.email || "No Email"}</span></div>
        <div className="flex items-center gap-2 text-xs text-slate-500"><Phone className="h-3 w-3 text-teal-600" /><span>{item.phone_number || creator?.phone_number || "No Phone"}</span></div>
      </div>
      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
        <Button variant="ghost" onClick={onOpenMaps} className="flex-col h-auto py-2.5 bg-slate-50 text-slate-500 rounded-xl border border-slate-100 hover:bg-slate-100 transition-colors">
          <Navigation className="h-4 w-4 mb-0.5" /><span className="text-[9px] font-bold uppercase">Verify Map</span>
        </Button>
        <Button variant="ghost" onClick={() => window.open(`/${type}/${item.id}`, "_blank")} className="flex-col h-auto py-2.5 bg-slate-50 text-slate-500 rounded-xl border border-slate-100 hover:bg-slate-100 transition-colors">
          <Eye className="h-4 w-4 mb-0.5" /><span className="text-[9px] font-bold uppercase">Live View</span>
        </Button>
      </div>
      {item.registration_number && (
        <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-xl border border-slate-100">
          <Landmark className="h-4 w-4 text-slate-400 flex-shrink-0" />
          <div><p className="text-[9px] font-black text-slate-400 uppercase">Reg / License No.</p><p className="text-xs font-black text-slate-700">{item.registration_number}</p></div>
        </div>
      )}
      <Button onClick={onApprove} disabled={isApproved} className="w-full py-6 rounded-xl text-sm font-bold text-white border-none shadow-md transition-all active:scale-95" style={{ background: isApproved ? "#94a3b8" : `linear-gradient(135deg, #2dd4bf 0%, ${TEAL} 100%)` }}>
        <CheckCircle2 className="mr-2 h-4 w-4" />
        {isApproved ? "Already Approved" : "Approve Entry"}
      </Button>
      {!isApproved && (
        <Button variant="ghost" onClick={onReject} className="w-full py-4 text-xs font-black uppercase tracking-widest text-red-500 hover:bg-red-50 rounded-xl">
          <XCircle className="mr-2 h-4 w-4" /> Reject Submission
        </Button>
      )}
      <Button variant="ghost" onClick={onToggleBan} className={`w-full py-4 text-xs font-black uppercase tracking-widest rounded-xl ${isBanned ? "text-green-600 hover:bg-green-50 border border-green-200" : "text-orange-500 hover:bg-orange-50 border border-orange-200"}`}>
...
const AdminReviewDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className={`sticky top-0 z-[90] transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-sm" : "bg-transparent"}`} style={{ paddingTop: "env(safe-area-inset-top, 0px)" }}>
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-sm font-black uppercase tracking-tight text-slate-700 hover:text-teal-600 transition-colors">
            <ArrowLeft className="h-5 w-5" /> Back to Admin
          </button>
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest text-white" style={{ background: CORAL }}>{type?.replace("_", " ")}</span>
      ...
      <div className="max-w-6xl mx-auto px-4 pt-4 pb-1 bg-background relative z-10">
        {isTrip && <span className="inline-block mb-2 px-3 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest text-white" style={{ background: CORAL }}>Trip</span>}
      ...
      <main className="container px-4 mt-5 relative z-10 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-[1.8fr,1fr] gap-6">
          <div className="space-y-6">
            <div className="lg:hidden"><AdminSideCard {...adminCardProps} /></div>

            {isAdventure && (
              <>
                {generalAmenities.length > 0 && <AmenitiesScroll amenities={generalAmenities} />}
                {item.facilities?.length > 0 && <div id="facilities-section"><InlineFacilitiesGrid facilities={item.facilities} /></div>}
                {item.activities?.length > 0 && <div id="activities-section"><InlineActivitiesGrid activities={item.activities} /></div>}
              </>
            )}
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      {item.inclusions?.length > 0 && (
                        <div>
                          ...
                          <ul className="space-y-1.5">{item.inclusions.map((inc: string, i: number) => <li key={i} className="flex items-start gap-2 text-sm text-emerald-700"><span className="text-emerald-500 mt-0.5">✓</span><span>{inc}</span></li>)}</ul>
                        </div>
                      )}
                      {item.exclusions?.length > 0 && (
                        <div>
                          <p className="text-[10px] font-black uppercase text-red-500 tracking-widest mb-2">✗ Not Included</p>
                          <ul className="space-y-1.5">{item.exclusions.map((exc: string, i: number) => <li key={i} className="flex items-start gap-2 text-sm text-red-600"><span className="text-red-400 mt-0.5">✗</span><span>{exc}</span></li>)}</ul>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs font-bold uppercase tracking-tight">
                      ...
                      <span className={item.allow_children === false ? "text-red-500" : "text-emerald-600"}>{item.allow_children === false ? "Not Allowed" : "Allowed"}</span>
                    </div>
            ...
            {item.description && (
              <section className="bg-white rounded-2xl px-5 py-4 shadow-sm border border-slate-100">
                <h2 className="text-base font-black uppercase tracking-tight mb-3" style={{ color: TEAL }}>{isAdventure ? "About this Place" : "About this Trip"}</h2>
                <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-line">{item.description}</p>
              </section>
            )}

            <MapSection name={item.name} latitude={item.latitude} longitude={item.longitude} location={item.location} country={item.country} mapLink={item.map_link || item.location_link} />
            ...
            <section className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  {isBanned && <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full bg-red-100 text-red-600 text-[9px] font-black uppercase"><Ban className="h-3 w-3" /> Banned</span>}
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase">Email</p>
                  <div className="flex items-center gap-1.5 mt-0.5"><Mail className="h-3 w-3 text-teal-600" /><p className="text-xs font-bold">{item.email || creator?.email || "No Email"}</p></div>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase">Phone</p>
                  <div className="flex items-center gap-1.5 mt-0.5"><Phone className="h-3 w-3 text-teal-600" /><p className="text-xs font-bold">{item.phone_number || creator?.phone_number || "No Phone"}</p></div>
                </div>
              </div>
              {item.registration_number && (
                <div className="mt-4 pt-4 border-t border-slate-100 flex items-center gap-2">
                  <Landmark className="h-4 w-4 text-slate-400" />
                  <div><p className="text-[9px] font-black text-slate-400 uppercase">Reg / License No.</p><p className="text-sm font-black">{item.registration_number}</p></div>
                </div>
              )}
            </section>
            ...
            {item.event_certificate_url && (
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                <h2 className="text-sm font-black uppercase tracking-tight mb-4 flex items-center gap-2" style={{ color: TEAL }}><FileImage className="h-4 w-4" /> Event Certificate / Permit</h2>
                <div className="rounded-xl overflow-hidden border-2 border-slate-100"><img src={item.event_certificate_url} alt="Event Certificate" className="w-full h-64 object-contain bg-slate-50" /></div>
                <Button variant="link" className="mt-2 text-[10px] font-black uppercase text-teal-600" onClick={() => window.open(item.event_certificate_url, "_blank")}>View Full Size →</Button>
              </div>
            )}

            {item.tra_license_url && (
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                <h2 className="text-sm font-black uppercase tracking-tight mb-4 flex items-center gap-2" style={{ color: TEAL }}><FileImage className="h-4 w-4" /> TRA License</h2>
                <div className="rounded-xl overflow-hidden border-2 border-slate-100"><img src={item.tra_license_url} alt="TRA License" className="w-full h-64 object-contain bg-slate-50" /></div>
                <Button variant="link" className="mt-2 text-[10px] font-black uppercase text-teal-600" onClick={() => window.open(item.tra_license_url, "_blank")}>View Full Size →</Button>
              </div>
            )}
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[100] md:hidden bg-black/90 backdrop-blur-xl border-t border-white/10 shadow-2xl" style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-xl" style={{ background: "#F59E0B" }}><ShieldAlert className="h-4 w-4 text-black" /></div>
            <div>
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={() => updateApprovalStatus("approved")} className="h-9 rounded-xl text-[10px] font-black px-4 border-none text-white" style={{ background: `linear-gradient(135deg, #2dd4bf 0%, ${TEAL} 100%)` }}>APPROVE</Button>
